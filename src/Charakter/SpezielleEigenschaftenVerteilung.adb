package body SpezielleEigenschaftenVerteilung is
   ...
   procedure SpezielleEigenschaftenVerteilung (Erhöhen : in Boolean; Wert : in Integer) is
   begin

      case Erhöhen is

         when True =>
            if Charaktere.Hauptcharakter.SpezielleEigenschaften (Wert) = True then
               Put_Line (To_Wide_String (Charaktere.SpezielleEigenschaftenNamen (Wert)) & " ist bereits ausgwewählt.");
            elsif Charaktere.Hauptcharakter.SonstigeWerte (7) = 0 then
         ...
         when False =>
            if Charaktere.Hauptcharakter.SpezielleEigenschaften (Wert) = False then
