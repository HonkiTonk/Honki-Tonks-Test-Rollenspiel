   ...
   is begin
      ...
      for Y in KartenArray'Range (2) loop
         ...
         for X in KartenArray'Range (3) loop
            ...
            if
              GlobaleVariablen.CharakterPosition (GlobaleVariablen.Ort).XPosition = X
              and
                GlobaleVariablen.CharakterPosition (GlobaleVariablen.Ort).YPosition = Y
            then
               Put (GlobaleVariablen.CharakterGrafik);
               
            elsif
              Karte (GlobaleVariablen.Ort, Y, X) = -1
            then
               New_Line;
               exit X_AchsenSchleife;
               
            elsif
              Karte (GlobaleVariablen.Ort, Y, X) = -2
            then
               New_Line;
